...
def text2atoms(response):
    ...
    response = response.split("<start_of_turn>model")[1].split(
        ". The"
    )[0].strip()
    print("response", response)
    tmp_atoms_array = response.strip("<end_of_turn>").split("\n")
    ...
    for example in tqdm(test_dataset):
        try:
            ...
            target_atoms = text2atoms(
                "<start_of_turn>model" + target_text
            )
...
def run(
    ...
    evaluate_and_save(
        ...
        tokenizer,
        test_datasets,
        output_path=f"{output_dir}/test_predictions3.json",
    )
